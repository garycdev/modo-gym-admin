...
@section('title')
    Predeterminado editar - Admin Panel
@endsection
    ...
    <div class="page-wrapper">
        <div class="page-content">
            ...
            <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
                <div class="breadcrumb-title pe-3">Editar rutina predeterminada</div>
                <div class="ps-3">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0 p-0">
                            <li class="breadcrumb-item"><a href="javascript:;"><i class="bx bx-home-alt"></i></a>
                            </li>
                            <li class="breadcrumb-item active" aria-current="page">Actualizar predeterminado</li>
                        </ol>
                    </nav>
                </div>
            </div>
                ...
                <div class="col-xl-6 mx-auto">
                    <div class="card">
                        <div class="card-header px-4 py-3">
                            <h5 class="mb-0">Editar rutina</h5>
                        </div>
                        <div class="card-body p-4">
                            <form class="row g-3 needs-validation" method="POST"
                                action="{{ route('admin.defecto.update', $defecto->def_id) }}">
                                @csrf
                                @method('PUT')
                                <div class="col-md-12">
                                    <label for="def_nombre" class="form-label">Nombre </label>
                                    <input type="text" id="def_nombre" name="def_nombre" class="form-control"
                                        placeholder="Nombre" value="{{ $defecto->def_nombre }}">
                                    @error('def_nombre')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                </div>
                                <div class="col-md-12">
                                    <label for="def_estado" class="form-label">Estado </label>
                                    <select name="def_estado" id="def_estado" class="form-control">
                                        <option value="ACTIVO" {{ $defecto->def_estado == 'ACTIVO' ? 'selected' : '' }}>
                                            ACTIVO</option>
                                        <option value="INACTIVO" {{ $defecto->def_estado == 'INACTIVO' ? 'selected' : '' }}>
                                            INACTIVO</option>
                                    </select>
                                    @error('def_estado')
                                        <span class="text-danger">{{ $message }}</span>
