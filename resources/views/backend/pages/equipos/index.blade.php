            ...
            <div class="card-body">
                <p class="float-right mb-2">
                    @if (Auth::guard('admin')->user()->can('equipo.create'))
                        <a class="btn btn-primary px-5 radius-30" href="{{ route('admin.equipos.create') }}">Crear Nuevo</a>
                    @endif
                </p>
                <br>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex flex-column align-items-center text-center">
                                        <img src="{{ asset($equipo->equi_imagen)}}" class="product-img-2" alt="product img"   width="120">
                                    </div>
                                </td>
                                ...
                                <td>
                                    @if (Auth::guard('admin')->user()->can('equipo.edit'))
                                        <a class="btn btn-success text-white" href="{{ route('admin.equipos.edit', $equipo->equi_id) }}">
                                            <i class='bx bxs-edit'></i>
                                        </a>
                                    @endif

                                    @if (Auth::guard('admin')->user()->can('equipo.delete'))
                                        <a class="btn btn-danger text-white" href="{{ route('admin.equipos.destroy', $equipo->equi_id ) }}"
                                        onclick="event.preventDefault(); document.getElementById('delete-form-{{ $equipo->equi_id  }}').submit();">
                                            <i class='bx bxs-trash'></i>
                                        </a>
                                        <form id="delete-form-{{ $equipo->equi_id }}" action="{{ route('admin.equipos.destroy', $equipo->equi_id ) }}" method="POST" style="display: none;">
                                            @method('DELETE')
                                            @csrf
                                        </form>
                                    @endif
